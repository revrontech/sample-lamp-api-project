...
class Helper {
	...
	public static function upload_file_json($picture, $path, $file = null, $company_id = null)
	{
		if($file == null) {
			$file_name = time();
			$file_name .= rand();
	...
	public static function encryptResponse($response = []) {
		...
		$responseExtraData = !empty($response['extradata']) ? $response['extradata'] : [] ;
		...
		return response()->json(['statusCode' => (string) $status, 'title' => $title, 'message' => $message, 'responseData' => $responseData, 'responseExtraData' => $responseExtraData, 'error' => $error], $status);
	}
	...
	public static function send_sms_firebase($plusCodeMobileNumber, $otp) {
	echo    "<script> 
		var config = {
			apiKey: \"AIzaSyCZQkYpAp7Ewl4mUsbKu00bGBoxj8Y_dTw\",
			authDomain: \"https://melanin-love.firebaseapp.com/__/auth/action?mode=<action>&oobCode=<code>\",
			databaseURL: \"https://melanin-love.firebaseio.com/\",
			projectId: \"melanin-love\",
			storageBucket: \"melanin-love.appspot.com\",
			messagingSenderId: \"320739367640\"
		};
		firebase.initializeApp(config);
 	   var window.recaptchaVerifier = true;
		firebase.auth().signInWithPhoneNumber(\"+919911725402\", window.recaptchaVerifier)
			.then(function (confirmationResult) {
				window.confirmationResult = confirmationResult;
				console.log(confirmationResult);
			});
	 
			window.confirmationResult.confirm('9911725402')
