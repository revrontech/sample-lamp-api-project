...
class ProviderServices { 
	...
	public function checkRequest($request) {
		...
		try{
			...
			// modified by : Asim
			$totalEarning = $this->totalEarning($provider);
			$totalTrip = $this->totalTrip($provider);
	...
	// created by : Asim
	public function totalEarning($Provider){
		
		$RideRequest = \App\Models\Transport\RideRequestPayment::select(DB::Raw('SUM(provider_pay) as salary'))->where('provider_id',$Provider->id)->first();
		$ServiceRequest = \App\Models\Service\ServiceRequestPayment::select(DB::Raw('SUM(provider_pay) as salary'))->where('provider_id',$Provider->id)->first();
		$OrderRequest = StoreOrder::with(['invoice' => function($query){  
						...
						])->select('id','delivery_address','pickup_address')->where('provider_id',$Provider->id)->first();
	...
	// created by : Asim
	public function totalTrip($Provider){
		$Ride = \App\Models\Transport\RideRequestPayment::where('provider_id',$Provider->id)->count();
		$Service = \App\Models\Service\ServiceRequestPayment::where('provider_id',$Provider->id)->count();
		$Order = StoreOrder::with(['invoice' => function($query){  
							$query->select('id','store_order_id','cart_details','updated_at' ); }
						])->select('id','delivery_address','pickup_address')->where('provider_id',$Provider->id)->count();

		$trip = $Ride+$Service+$Order;
		return $trip;
	}
