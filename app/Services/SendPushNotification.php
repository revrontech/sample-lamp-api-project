    ...
    */
    ...
    public function sendPushToUser($user_id, $topic, $push_message, $title = null, $data = null){
        ...
        try{
            ...
            if($user->device_token != ""){
                shell_exec('curl -X POST --header "Authorization: key='.$settings->site->android_push_key.'" --header "Content-Type: application/json" https://fcm.googleapis.com/fcm/send -d "{\"to\":\"'.$user->device_token.'\",\"priority\":\"high\",\"data\":{\"custom\":{\"message\":{\"topic\":\"'.$topic.'\",\"notification\":{\"body\": \"'.stripslashes($push_message).'\",\"title\":\"'.$title.'\"},\"data\":\"'.$data.'\"}}},\"notification\":{\"body\": \"'.stripslashes($push_message).'\",\"title\":\"'.$title.'\"}}"');
    ...
    public function sendPushToProvider($provider_id, $topic, $push_message, $title = null, $data = null){
        ...
        try{ 
            ...
            if($user->device_token != ""){
                shell_exec('curl -X POST --header "Authorization: key='.$settings->site->android_push_key.'" --header "Content-Type: application/json" https://fcm.googleapis.com/fcm/send -d "{\"to\":\"'.$user->device_token.'\",\"priority\":\"high\",\"data\":{\"custom\":{\"message\":{\"topic\":\"'.$topic.'\",\"notification\":{\"body\": \"'.stripslashes($push_message).'\",\"title\":\"'.$title.'\"},\"data\":\"'.$data.'\"}}},\"notification\":{\"body\": \"'.stripslashes($push_message).'\",\"title\":\"'.$title.'\"}}"');                //dispatch(new PushNotificationJob($topic, $push_message, $title, $data, $user, $settings, 'provider'));
            }
