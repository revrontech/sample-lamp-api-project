...
use Actions;
	...
	public function get_ride_lost_item(Request $request,$id) {
		$ride_lost_item = RideLostItem::where('company_id',Auth::guard('user')->user()->company_id)
								->where('ride_request_id',$id)
								->first();
		$ride_lost_item->created_time=(\Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $ride_lost_item->created_at, 'UTC'))->setTimezone(Auth::guard('user')->user()->timezone)->format(Helper::dateFormat());					
