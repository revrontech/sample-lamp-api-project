...
class VehicleController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if($request->has('page') && $request->page == 'all') {
         ...
         $resArr = [];
         $resArr['all'] = RideDeliveryVehicle::with('ride_type')->where('company_id', Auth::user()->company_id)->count();
         $resArr['active'] = RideDeliveryVehicle::with('ride_type')->where('company_id', Auth::user()->company_id)->where('status', '1')->count();
         $resArr['inactive'] = RideDeliveryVehicle::with('ride_type')->where('company_id', Auth::user()->company_id)->where('status', '0')->count();
        
        return Helper::encryptResponse(['data' => $data, 'extradata'=>$resArr]);
    } 
    ...
    public function store(Request $request)
    {
        ...
        try {
                ...
                if($request->hasFile('vehicle_image')) {
                    $rideDeliveryVehicle->vehicle_image = Helper::upload_file_json($request->file('vehicle_image'), 'vehicle/image');
                }
                if($request->hasFile('vehicle_marker')) {
                    $rideDeliveryVehicle->vehicle_marker = Helper::upload_file_json($request->file('vehicle_marker'), 'vehicle/marker');
                }
