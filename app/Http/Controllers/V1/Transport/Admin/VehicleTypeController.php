...
class VehicleTypeController extends Controller
{
    ...
    public function index(Request $request) 
    {
        ...
        if($request->has('page') && $request->page == 'all') {
         ...
         $resArr = [];
         $resArr['all'] = RideType::where('company_id', Auth::user()->company_id)->count();
         $resArr['active'] = RideType::where('company_id', Auth::user()->company_id)->where('status', '1')->count();
         $resArr['inactive'] = RideType::where('company_id', Auth::user()->company_id)->where('status', '0')->count();
