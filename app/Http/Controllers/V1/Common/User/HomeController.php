...
class HomeController extends Controller
{
    ...
    public function menusIgnoreAuth(Request $request) {
        $user = Auth::guard('user')->user();
    ...
    public function countries1(Request $request) {
        $company_id = base64_decode($request->salt_key);
        $country_list = CompanyCountry::with(['companyCountryState' => function($q) use($company_id) {  $q->where('company_id', $company_id); }])->has('companyCountryState')->where('company_id', $company_id )->where('status', 1)->get();
        $countries = [];
        ...
        foreach ($country_list as $country) {
            ...
            $object->status = $country->country->status;
            $object->timezone = $country->country->timezone;
            foreach ($country->companyCountryState as $value) {
    ...
    public function citiesIgnoreAuth(Request $request)
    {
   ...
   // shazz
    public function promocodeIgnoreAuth(Request $request)
    {

        $promocode = Promocode::with('service')->whereHas('service' ,function($query){  
                                $query->where('status',1);  
                            })->whereDate('expiration','>=',Carbon::today())->orderby('id','desc')->get();
