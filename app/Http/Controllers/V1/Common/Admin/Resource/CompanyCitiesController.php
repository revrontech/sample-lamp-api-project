...
class CompanyCitiesController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if($request->has('page') && $request->page == 'all') {
         ...
         $resArr = [];
         $resArr['all'] = CompanyCity::with('country','city','state','city_service')->where('company_id', Auth::user()->company_id)->count();
         $resArr['active'] = CompanyCity::with('country','city','state','city_service')->where('company_id', Auth::user()->company_id)->where('status', '1')->count();
         $resArr['inactive'] = CompanyCity::with('country','city','state','city_service')->where('company_id', Auth::user()->company_id)->where('status', '0')->count();
