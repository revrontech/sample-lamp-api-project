...
class UserController extends Controller
{
    ...
    //    $datum =  array_merge($datum, $resArr);
        
        // echo "<pre>"; print_r($datum->toArray()); die;
        $resArr['all'] = User::where('company_id', Auth::user()->company_id)->count();
         $resArr['active'] = User::where('company_id', Auth::user()->company_id)->where('status', '1')->count();
         $resArr['inactive'] = User::where('company_id', Auth::user()->company_id)->where('status', '0')->count(); 
    ...
    public function store(Request $request)
    {
        ...
        try{
            ...
            if($request->hasFile('picture')) {
                $user->picture = Helper::upload_file_json($request->file('picture'), 'user/profile', $user->id.'.png');
            }
