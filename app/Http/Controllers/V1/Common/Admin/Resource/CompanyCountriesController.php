...
class CompanyCountriesController extends Controller
{
    ...
    {   
        ...
        $resArr = [];
        $resArr['all'] = CompanyCountry::with('country')->where('company_id', Auth::user()->company_id)->count();
        $resArr['active'] = CompanyCountry::with('country')->where('company_id', Auth::user()->company_id)->where('status', '1')->count();
        $resArr['inactive'] = CompanyCountry::with('country')->where('company_id', Auth::user()->company_id)->where('status', '0')->count();
