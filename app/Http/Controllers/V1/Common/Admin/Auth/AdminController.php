	...
	{    
		...
		if($setting != null) {
			...
			if($request->hasFile('site_icon')) {
				$site_icon = Helper::upload_file_json($request->file('site_icon'), 'site', 'site_icon.'.$request->file('site_icon')->extension());
				$data['site']['site_icon'] = $site_icon;
			}

			if($request->hasFile('site_logo')) {
				$site_logo = Helper::upload_file_json($request->file('site_logo'), 'site', 'site_logo.'.$request->file('site_logo')->extension());
				$data['site']['site_logo'] = $site_logo;
			}

			if($request->hasFile('user_pem')) {
				$user_pem = Helper::upload_file_json($request->file('user_pem'), 'apns', 'user.pem');
				$data['site']['user_pem'] = $user_pem;
			}

			if($request->hasFile('provider_pem')) {
				$provider_pem = Helper::upload_file_json($request->file('provider_pem'), 'apns', 'provider.pem');
				$data['site']['provider_pem'] = $provider_pem;
			}

			if($request->hasFile('shop_pem')) {
				$shop_pem = Helper::upload_file_json($request->file('shop_pem'), 'apns', 'shop.pem');
