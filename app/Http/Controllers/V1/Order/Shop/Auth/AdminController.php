...
class AdminController extends Controller
{
    ...
    public function total_ordersShop()
    {
       try{
           ...
           $storeorder = StoreOrder::where('store_id',Auth::guard('shop')->user()->id);
            ...
            $data['received_data'] = $storeorder->count();
            $data['delivered_data'] = $storeorder->where('status','COMPLETED')->count();
            $sdata=$storeorder->with('user','orderInvoice','provider')->orderby('id','DESC')->get();
              foreach ($sdata as $key => $sd) {
                    $sdata[$key]['pickup_address'] = json_decode($sd['pickup_address']);
                    $sdata[$key]['delivery_address'] = json_decode($sd['delivery_address']);
                    //$sdata[$key]['order_invoice']['cart_details'] = json_decode($sd['order_invoice']['cart_details']);

              }
          $data['recent_data']=$sdata;
