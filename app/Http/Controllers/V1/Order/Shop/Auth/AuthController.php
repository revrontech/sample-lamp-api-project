...
class AuthController extends Controller
{
    ...
    public function login(Request $request) {
        ...
        if($User->status == 0){
            return Helper::getResponse(['status' => 422, 'message' => 'your account is disabled please wait for admin approval.']);
        }
        ...
        $ud = Auth::guard('shop')->user();
        $ud['wallet_balance'] = (String)$ud['wallet_balance'];
        return Helper::getResponse(['data' => ["token_type" => "Bearer", "expires_in" => (config('jwt.ttl', '0') * 60), "access_token" => $token, 'user' => $ud]]);
    ...
    public function verifyNumber(Request $request){
        ...
        $request->merge([
            'loginUser' => $this->cusencrypt($request->mobile,env('DB_SECRET'))            
        ]);
        
        $where = ['contact_number'=>$request->loginUser];

        $userQuery = Store::where($where)->first();
       	if($userQuery==null){
       		return Helper::getResponse(['status' => 422, 'message'=>'Number not found','data'=>[]]);              
       	}else{
       		return Helper::getResponse(['status' => 200, 'message'=>'success','data'=>$userQuery]);              
       	}
    }
    ...
    public function resetPasswordShop(Request $request) {
        ...
        $responseData = $request->all();
        try {
            //$username = isset($request->email)?$request->email:'';
            $newpassword = isset($request->password)?$request->password:'';
            // $request->merge([
            //     'loginUser' => $this->cusencrypt($username,env('DB_SECRET'))            
            // ]);
            ...
            $userQuery = Store::findOrFail($request->id);
                //User Not Exists
            $validator  = Validator::make([],[],[]);
            if($userQuery == null) {         
                $validator->errors()->add('Result', 'User not found');
