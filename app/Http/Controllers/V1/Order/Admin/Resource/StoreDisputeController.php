...
class StoreDisputeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $resArr['all'] = StoreOrderDispute::with('request','user','provider')->where('company_id', Auth::user()->company_id)->count();
        $resArr['active'] = StoreOrderDispute::with('request','user','provider')->where('company_id', Auth::user()->company_id)->where('status', 'open')->count();
        $resArr['inactive'] = StoreOrderDispute::with('request','user','provider')->where('company_id', Auth::user()->company_id)->where('status', 'closed')->count();
