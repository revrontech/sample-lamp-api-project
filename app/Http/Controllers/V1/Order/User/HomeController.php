...
class HomeController extends Controller
{
    ...
    public function shoprequestHistoryShop(Request $request)
    {
        ...
        try {
            ...
            if($request->has('limit') && $request->has('type')) {
             $data=$datum->where("status",$request->type)->take($request->limit)->offset($request->offset)->get(); 
               ...
               $data = $datum->whereIn('status',$history_status)->orderby('id','desc')->get();    
            }  
            foreach ($data as $k => $d) {
                $data[$k]['pickup_address'] =  json_decode($d->pickup_address,true);
                $data[$k]['delivery_address'] =  json_decode($d->delivery_address,true);
                $data[$k]['store'] =  Store::findOrFail($d->store_id);
                //$data[$k]['user_data'] =  User::findOrFail($d->user_id);
                //$data[$k]['provider_data'] =  Store::findOrFail($d->store_id);
